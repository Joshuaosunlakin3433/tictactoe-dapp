...
(define-public (create-game
    ...
    (let (
            ;; 1. Get the Game ID to use for creation of this new game
            (game-id (var-get latest-game-id))
            ;; 2. The initial starting board for the game with all cells empty
            (starting-board (list u0 u0 u0 u0 u0 u0 u0 u0 u0))
            ;; 3. Updated board with the starting move played by the game creator (X)
        ...
        ;; 4. Ensure that user has put up a bet amount greater than the minimum
        (asserts! (> bet-amount u0) (err ERR_MIN_BET_AMOUNT))
        ;; 5. Ensure that the move being played is an `X`, not an `O`
...
(define-public (join-game
    ...
    (let (
            ;; 1. Load the game data for the game being joined, throw an error if Game ID is invalid
...
;; Back to contracts
;; Making a move as a player- basically the contract that defines playing properly now
